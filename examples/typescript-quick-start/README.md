...
First, initialize the Metorial client and create a deployment for the built-in web search provider. `metorial-search` is a special provider slug that works out of the box — no dashboard configuration needed:
...
Then open a streaming session. `withProviderSession` connects to Metorial's MCP servers and provides `tools` formatted for the AI SDK. The `streaming: true` flag keeps the connection alive for the duration of the stream:
...
Inside the session, call `streamText()` from the Vercel AI SDK. The AI SDK handles the tool call loop automatically — when Claude wants to search the web, it calls the MCP tools, gets results, and continues generating. `stepCountIs(10)` limits the number of tool call rounds:
...
Finally, stream the text to stdout as it arrives:
