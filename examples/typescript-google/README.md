...
```bash
bun install
METORIAL_API_KEY=... GOOGLE_API_KEY=... bun start
```
...
Open a session with `metorialGoogle`, which formats MCP tools as Gemini function declarations. Gemini expects tools in the `config.tools` field:
...
Gemini returns function calls as `functionCall` parts inside `response.candidates`. Extract them and execute via `session.callTools()`:
...
Note: this example does a single round of tool calls. For a full multi-turn loop (like the Anthropic/OpenAI examples), you'd feed the tool results back into `contents` and call `generateContent` again.
